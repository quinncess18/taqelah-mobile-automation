...
**iOS:** Simulator path live on GHA `macos-14` (iPhone 15, iOS 17.5). All 14 iOS-applicable modules verified green: §0 Smoke, §1 Auth, §2 Catalog, §3 Nav Main (03/01), §3 Gestures (03/02), §3 WebView (03/03), §3 Dialogs (03/04), §3 Form (03/05), §3 Tabs (03/08), §3 Location (03/10), §3 Dark Mode (03/11), §4 Products (04/01), §4 Cart (04/02), §4 Checkout (04/03), §16 Regression. iOS-incompatible modules omitted from the iOS lane: §3 Permissions (03/06), §3 Notifications (03/07), §3 Camera (03/09). Real-device path (BrowserStack/Sauce) parked — requires a device-signed `.ipa` not currently published.
...
| §3 Dialogs (03/04) | ✅ | Verified (run 26293933727). D01–D08 green; D08 full-screen close via nameless-button predicate; date/time pickers via canvas-angle taps. |
...
| §4 Cart (04/02) | ✅ | Verified (run 26446967407, S01–S05, clean). iOS Flutter virtualizes off-screen cart rows as zero-size `XCUIElementTypeImage` nodes that the raw a11y tree lists FIRST in DOM order — read paths (`getLine`/`_readVisibleSnapshot`) and click paths (`_lineButtons`) must agree on which line is "N", or `tapPlus(0)` clicks one line and polls qty on another. Fixes: `_orderedLineElements()` filters ghosts + sorts by Y as the single source of truth for line indexing; iOS `_scrollCartToTop` swipes content down until topmost Y stabilizes (no UiScrollable on iOS); `_ensureLineVisible` is called at the start of every tap method to bring the target line into the viewport. |
| §4 Checkout (04/03) | ✅ | Verified (run 26562046402, K01–K04, clean). K02 fill→Review→Place Order race fixed by `addValue + '\n'` per-field (Flutter `onSubmitted` commits `TextEditingController` before next focus) + self-heal wrapper that re-fills if Review doesn't appear in 3s. K04 back-from-Review uses `deviceBack()` (raw `driver.back()` is a no-op on iOS Flutter routes). |
| §16 Regression | ✅ | Verified (run 26562046402, E01, clean). Full E2E single-product journey from cold launch. iOS reset uses `mobile: terminateApp + clearApp + launchApp` (terminate+launch alone preserves Flutter's cart on Simulator; `clearApp` wipes app data for true cold-launch parity with Android's `pm clear`). |
