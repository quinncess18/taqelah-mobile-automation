...
| §3 Form (03/05) | ⚠️ | Bring-up in flight. Text fields are Flutter `Key()`-based (no iOS `accessibilityIdentifier`) — rebuild via `~labelText` / class-chain from diagnostic XML. Visible-text labels (Size/Submit/Reset/Category/errors) expected to carry over. |
